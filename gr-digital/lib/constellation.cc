...
void constellation::gen_soft_dec_lut(int precision, float npwr)
{
    ...
    int numpts = static_cast<int>(d_lut_scale);
    float maxd = d_maxamp * d_padding;
    float slope = (2 * maxd) / (numpts - 1);
    d_soft_dec_lut.clear();
    for (int im = 0; im < numpts; ++im) {
        for (int re = 0; re < numpts; ++re) {
            gr_complex pt(slope * re - maxd, slope * im - maxd);
...
std::vector<float> constellation::soft_decision_maker(gr_complex sample)
{
    if (has_soft_dec_lut()) {
        ...
        int npts = static_cast<int>(d_lut_scale);
        float slope = (npts - 1) / (2 * maxd);
        int index_real = std::round(slope * (maxd + sample.real()));
        index_real = std::clamp(index_real, 0, npts - 1);
        int index_imag = std::round(slope * (maxd + sample.imag()));
        index_imag = std::clamp(index_imag, 0, npts - 1);
        size_t index = index_imag * npts + index_real;
        if (index >= d_soft_dec_lut.size()) {
            index = d_soft_dec_lut.size() - 1;
        }
...
void constellation::max_min_axes()
{
    ...
    for (const gr_complex& pt : d_constellation) {
        d_maxamp = std::max({ d_maxamp, std::fabs(pt.real()), std::fabs(pt.imag()) });
    }
}

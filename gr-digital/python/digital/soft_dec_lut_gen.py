...
def soft_dec_table_generator(soft_dec_gen, prec, Es=1):
    ...
    | sqrt(Es) multiplied by the AXIS_PADDING. We then calculate
    ...
    npts = 2**prec
    maxd = AXIS_PADDING * numpy.sqrt(Es)
...
def soft_dec_table(constel, symbols, prec, npwr=1):
    ...
    max_amp = numpy.max(numpy.abs(constel))
...
def calc_soft_dec_from_table(sample, table, prec, max_amp=1):
    ...
    Args:
        ...
        max_amp: The maximum amplitude for all points in the
                 constellation

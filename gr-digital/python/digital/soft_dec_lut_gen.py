...
def soft_dec_table_generator(soft_dec_gen, prec, Es=1, max_amp=1):
    """
    | Builds a LUT that is a list of lists. The internal list represents
    ...
    | max_amp multiplied by the AXIS_PADDING. We then calculate
    | coordinates from -A to A with 2^prec points for both the x and y
    | axes. We then sample starting at (-A, -A) and move left to right
    | on the x-axis and then move up a row on the y-axis. For every
    | point in this sampled space, we calculate the soft decisions for
    | the given constellation/mapping. This is done by passing in the
    | function 'soft_dec_gen' as an argument to this function. This
    | takes in the x/y coordinates and outputs the soft decisions. These
    | soft decisions are stored into the list at the index from the
    | above table as an internal list.
    |
    ...
    |
    | Args:
    |     soft_dec_gen: The soft decision function for a given constellation
    |     prec: The bit precision resulting in a table length 4 ** prec
    |     Es: The maximum energy per symbol of the constellation
    |     max_amp: The largest axis value for all constellation points
    |
    | Returns:
    |     A lookup table of soft decisions
    """

    npts = 2**prec
    maxd = AXIS_PADDING * max_amp
...
def const_normalization(constel, method="POWER"):
    norm = numpy.array(constel)
    if method == "POWER":
        avgpwr = numpy.mean(norm.real * norm.real + norm.imag * norm.imag)
        norm = norm / numpy.sqrt(avgpwr)
    return norm
...
def calc_soft_dec_from_table(sample, table, prec, max_amp=1):
    ...
    Args:
        ...
        max_amp: The maximum axis value for all points in the
                 constellation, as calculated by min_max_axes

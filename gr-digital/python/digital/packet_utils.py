...
default_access_code = \
    conv_packed_binary_string_to_1_0_string(
        b'\xAC\xDD\xA4\xE2\xF2\x8C\x20\xFC')
default_preamble = \
...
def make_packet(payload, samples_per_symbol, bits_per_symbol,
    ...
    if not is_1_0_string(preamble):
        raise ValueError(
            "preamble must be a string containing only 0's and 1's (%r)" % (preamble,))

    if not is_1_0_string(access_code):
        raise ValueError(
            "access_code must be a string containing only 0's and 1's (%r)" % (access_code,))

    if not whitener_offset >= 0 and whitener_offset < 16:
        raise ValueError(
            "whitener_offset must be between 0 and 15, inclusive (%i)" % (whitener_offset,))
    ...
    if pad_for_usrp:
        pkt = pkt + (_npadding_bytes(len(pkt),
                     int(samples_per_symbol), bits_per_symbol) * b'\x55')
...
def _npadding_bytes(pkt_byte_len, samples_per_symbol, bits_per_symbol):
    ...
    """
    modulus = 128
    byte_modulus = (modulus // 8) // math.gcd(modulus // 8,

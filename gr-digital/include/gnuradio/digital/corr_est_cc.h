...
public:
     ...
     *                          is actually 1 - Probability of False Alarm (under some
     * inaccurate assumptions). The code performs the check |r[k]|^2 + |r[k+1]|^2 <>
     * -log(1-threshold)*2*E, where r[k] is the correlated incoming signal, and E is the
     * average sample energy of the correlated signal. For ABSOLUTE threshold method, this
     * parameter sets the threshold to a fraction of the maximum squared autocorrelation.
     *                          The code performs the check |r[k]|^2 <> threshold * R^2,
     *                          where R is the precomputed max autocorrelation of the
     * given sync word. Default is 0.9 \param threshold_method  Method for computing
     * threshold.
     *
     */

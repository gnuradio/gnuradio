...
{
public:
    using sptr = std::shared_ptr<ofdm_serializer_vcc>;

    /*!
     * \param fft_len FFT length
     * \param occupied_carriers See ofdm_carrier_allocator_cvc.
     * \param len_tag_key The key of the tag identifying the length of the input frame in
     * OFDM symbols. \param packet_len_tag_key The key of the tag identifying the number
     * of complex symbols in this packet. \param symbols_skipped If the first symbol is
     * not allocated as in \p occupied_carriers[0], set this \param carr_offset_key When
     * this block should correct a carrier offset, specify the tag key of the offset here
     * (not necessary if following an ofdm_frame_equalizer_vcvc) \param input_is_shifted
     * If the input has the DC carrier on index 0 (i.e. it is not FFT shifted), set this
     * to false
     */
     ...
     * \param packet_len_tag_key The key of the tag identifying the number of complex
     * symbols in this packet. \param symbols_skipped If the first symbol is not allocated
     * as in \p occupied_carriers[0], set this \param carr_offset_key When this block
     * should correct a carrier offset, specify the tag key of the offset here (not
     * necessary if following an ofdm_frame_equalizer_vcvc) \param input_is_shifted If the
     * input has the DC carrier on index 0 (i.e. it is not FFT shifted), set this to false
     */
    static sptr make(const gr::digital::ofdm_carrier_allocator_cvc::sptr& allocator,
                     const std::string& packet_len_tag_key = "",
                     int symbols_skipped = 0,
                     const std::string& carr_offset_key = "",
                     bool input_is_shifted = true);

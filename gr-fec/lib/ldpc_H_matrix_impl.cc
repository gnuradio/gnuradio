    ...
    : fec_mtrx_impl()
{
    ...
    gsl_matrix_uchar* temp_mtrx = gsl_matrix_uchar_alloc(d_num_rows, d_num_cols);
    gsl_matrix_uchar_memcpy(temp_mtrx, (gsl_matrix_uchar*)(x.get()));
...
const gsl_matrix_uchar* ldpc_H_matrix_impl::A() const
{
    const gsl_matrix_uchar* A_ptr = &d_A_view.matrix;
    return A_ptr;
}

const gsl_matrix_uchar* ldpc_H_matrix_impl::B() const
{
    const gsl_matrix_uchar* B_ptr = &d_B_view.matrix;
    return B_ptr;
}

const gsl_matrix_uchar* ldpc_H_matrix_impl::D() const
{
    const gsl_matrix_uchar* D_ptr = &d_D_view.matrix;
    return D_ptr;
}

const gsl_matrix_uchar* ldpc_H_matrix_impl::E() const
{
    const gsl_matrix_uchar* E_ptr = &d_E_view.matrix;
    return E_ptr;
}

const gsl_matrix_uchar* ldpc_H_matrix_impl::T() const
{
    const gsl_matrix_uchar* T_ptr = &d_T_view.matrix;
    return T_ptr;
}

const gsl_matrix_uchar* ldpc_H_matrix_impl::phi_inverse() const
{
    const gsl_matrix_uchar* phi_inverse_ptr = d_phi_inverse_ptr;
...
void ldpc_H_matrix_impl::set_parameters_for_encoding()
{
    ...
    // T submatrix
    d_T_view =
        gsl_matrix_uchar_submatrix((gsl_matrix_uchar*)(d_H_sptr.get()), 0, 0, t, t);

    gsl_matrix_uchar* d_T_inverse_ptr;
    try {
        d_T_inverse_ptr = calc_inverse_mod2(&d_T_view.matrix);
    } catch (char const* exceptionString) {
        d_logger.error("Error in set_parameters_for_encoding while "
    ...
    // E submatrix
    d_E_view = gsl_matrix_uchar_submatrix(
        (gsl_matrix_uchar*)(d_H_sptr.get()), t, 0, d_gap, d_n - d_k - d_gap);

    // A submatrix
    d_A_view =
        gsl_matrix_uchar_submatrix((gsl_matrix_uchar*)(d_H_sptr.get()), 0, t, t, d_gap);

    // C submatrix (used to find phi but not during encoding)
    gsl_matrix_uchar_view C_view = gsl_matrix_uchar_submatrix(
        (gsl_matrix_uchar*)(d_H_sptr.get()), t, t, d_gap, d_gap);

    // These are just temporary matrices used to find phi.
    gsl_matrix_uchar* temp1 =
        gsl_matrix_uchar_alloc(d_E_view.matrix.size1, d_T_inverse_ptr->size2);
    mult_matrices_mod2(temp1, &d_E_view.matrix, d_T_inverse_ptr);

    gsl_matrix_uchar* temp2 = gsl_matrix_uchar_alloc(temp1->size1, d_A_view.matrix.size2);
    mult_matrices_mod2(temp2, temp1, &d_A_view.matrix);

    // Solve for phi.
    gsl_matrix_uchar* phi = gsl_matrix_uchar_alloc(C_view.matrix.size1, temp2->size2);
    add_matrices_mod2(phi, &C_view.matrix, temp2);

    // If phi has at least one nonzero entry, try for inverse.
    if (gsl_matrix_uchar_max(phi)) {
        try {
            gsl_matrix_uchar* inverse_phi = calc_inverse_mod2(phi);
    ...
    // B submatrix
    d_B_view = gsl_matrix_uchar_submatrix(
        (gsl_matrix_uchar*)(d_H_sptr.get()), 0, t + d_gap, t, d_n - d_gap - t);

    // D submatrix
    d_D_view = gsl_matrix_uchar_submatrix(
        (gsl_matrix_uchar*)(d_H_sptr.get()), t, t + d_gap, d_gap, d_n - d_gap - t);

    // Free memory
    gsl_matrix_uchar_free(temp1);
    gsl_matrix_uchar_free(temp2);
    gsl_matrix_uchar_free(phi);
    gsl_matrix_uchar_free(d_T_inverse_ptr);
}

void constexpr ldpc_H_matrix_impl::back_solve_mod2(gsl_matrix_uchar* x,
                                                   ...
                                                   const gsl_matrix_uchar* y)
{
    ...
    const int num_rows = (*U).size1;
    const int num_cols_U = (*U).size2;

    // Back solve
    for (int i = num_rows - 1; i >= 0; i--) {
        // x[i] = y[i]
        gsl_matrix_uchar_set(x, i, 0, gsl_matrix_uchar_get(y, i, 0));

        for (int j = i + 1; j < num_cols_U; j++) {
            const int U_i_j = gsl_matrix_uchar_get(U, i, j);
            const int x_i = gsl_matrix_uchar_get(x, i, 0);
            const int x_j = gsl_matrix_uchar_get(x, j, 0);
            const int temp1 = (U_i_j * x_j) & 1;
            const int temp2 = (x_i + temp1) & 1;
            gsl_matrix_uchar_set(x, i, 0, temp2);
        }
        // Perform x[i] /= U[i,i], GF(2) operations
        int U_i_i = gsl_matrix_uchar_get(U, i, i);
        int x_i = gsl_matrix_uchar_get(x, i, 0);
        // 0 divided by anything is 0 in GF2, so, hm, only care about 1
        if (x_i == 1) {
            if (U_i_i == 1) {
                gsl_matrix_uchar_set(x, i, 0, 1);
            } else {
                // x[i] = 1 and U[i][i] = 0 -> a/0 = undef
                gr::logger("ldpc_H_matrix_impl")
                    .error("Error in back_solve_mod2, division between x[{0}]={1} and "
                           "U[{0}]={2} not defined.",
                           i,
                           x_i,
                           U_i_i);
                                ...
                                const unsigned char* inbuffer) const
{
    ...
    gsl_matrix_uchar *s, *p1, *p2;
    gsl_matrix_uchar *temp1, *temp2, *temp3, *temp4, *temp5, *temp6, *temp7;

    unsigned int index, k = d_k;
    s = gsl_matrix_uchar_alloc(k, 1);
    ...
    temp1 = gsl_matrix_uchar_alloc(B()->size1, s->size2);
    mult_matrices_mod2(temp1, B(), s);

    temp2 = gsl_matrix_uchar_alloc(T()->size1, 1);
    back_solve_mod2(temp2, T(), temp1);

    temp3 = gsl_matrix_uchar_alloc(E()->size1, temp2->size2);
    mult_matrices_mod2(temp3, E(), temp2);

    temp4 = gsl_matrix_uchar_alloc(D()->size1, s->size2);
    mult_matrices_mod2(temp4, D(), s);

    temp5 = gsl_matrix_uchar_alloc(temp4->size1, temp3->size2);
    add_matrices_mod2(temp5, temp4, temp3);

    p2 = gsl_matrix_uchar_alloc(phi_inverse()->size1, temp5->size2);
    ...
    temp6 = gsl_matrix_uchar_alloc(A()->size1, p2->size2);
    mult_matrices_mod2(temp6, A(), p2);

    temp7 = gsl_matrix_uchar_alloc(temp6->size1, temp1->size2);
    add_matrices_mod2(temp7, temp6, temp1);

    p1 = gsl_matrix_uchar_alloc(T()->size1, 1);
    ...
    for (index = 0; index < p1_length; index++) {
        int value = gsl_matrix_uchar_get(p1, index, 0);
        outbuffer[index] = value;
    }
    for (index = 0; index < p2_length; index++) {
        int value = gsl_matrix_uchar_get(p2, index, 0);
        outbuffer[p1_length + index] = value;
    }
    for (index = 0; index < k; index++) {
        int value = gsl_matrix_uchar_get(s, index, 0);
    ...
    gsl_matrix_uchar_free(temp1);
    gsl_matrix_uchar_free(temp2);
    gsl_matrix_uchar_free(temp3);
    gsl_matrix_uchar_free(temp4);
    gsl_matrix_uchar_free(temp5);
    gsl_matrix_uchar_free(temp6);
    gsl_matrix_uchar_free(temp7);
    gsl_matrix_uchar_free(p1);
    gsl_matrix_uchar_free(p2);
}
                                ...
                                unsigned int max_iterations) const
{
    ...
    gsl_matrix_uchar* x = gsl_matrix_uchar_alloc(n, 1);
    for (index = 0; index < n; index++) {
        double value = inbuffer[index] > 0 ? 1.0 : 0.0;
        gsl_matrix_uchar_set(x, index, 0, value);
    }
    ...
    while ((count < max_iterations) && !found_word) {
        ...
        for (i = 0; i < rows_of_interest_in_H.size(); i++) {
            ...
            for (col_num = 0; col_num < n; col_num++) {
                double value = gsl_matrix_uchar_get(H(), row_num, col_num);
                if (value > 0) {
        ...
        for (index = 0; index < n; index++) {
            if (counts[index] == max) {
                unsigned int value = gsl_matrix_uchar_get(x, index, 0);
                unsigned int new_value = value ^ 1;
                gsl_matrix_uchar_set(x, index, 0, new_value);
            }
        }
    ...
    if (parity_bits_come_last()) {
        ...
        for (index = 0; index < frame_size; index++) {
            ...
            int value = gsl_matrix_uchar_get(x, i, 0);
            outbuffer[index] = value;
        }
    }
...
ldpc_H_matrix_impl::~ldpc_H_matrix_impl()
{
    ...
    gsl_matrix_uchar_free(d_phi_inverse_ptr);
}
} /* namespace code */
} /* namespace fec */
} /* namespace gr */

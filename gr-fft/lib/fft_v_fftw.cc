...
fft_v_fftw<T, forward>::fft_v_fftw(int fft_size,
    ...
    : sync_block("fft_v_fftw",
                 io_signature::make(
                     1, 1, fft_size * sizeof(typename fft_inbuf<T, forward>::type)),
                 io_signature::make(
                     1, 1, fft_size * sizeof(typename fft_outbuf<T, forward>::type))),
      d_fft_size(fft_size),
...
void fft_v_fftw<gr_complex, true>::fft_and_shift(const gr_complex* in, gr_complex* out)
{
    if (!d_window.empty()) {
        ...
        volk_32fc_32f_multiply_32fc(
            &dst[0], in, &d_window[0], sizeof(gr_complex) * d_fft_size);
    } else {
        memcpy(d_fft.get_inbuf(), in, d_fft_size);
    }
    d_fft.execute();
    if (d_shift) {
...
{
    const typename fft_inbuf<T, forward>::type* in =
        (const typename fft_inbuf<T, forward>::type*)input_items[0];
    typename fft_outbuf<T, forward>::type* out =
        (typename fft_outbuf<T, forward>::type*)output_items[0];

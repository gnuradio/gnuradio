...
protected:
    ...
    int d_latest_n;
    int d_latest_m;
    static auto get_acc_type() { return tap_type(0) * i_type(0); }
    using acc_type = decltype(get_acc_type());
...
//
template <class i_type, class o_type, class tap_type>
o_type iir_filter<i_type, o_type, tap_type>::filter(const i_type input)
{
    decltype(d_fftaps[0] * input) acc;
...
template <class i_type, class o_type, class tap_type>
void iir_filter<i_type, o_type, tap_type>::filter_n(o_type output[],
                                                    const i_type input[],
                                                    long n)
{
    for (int i = 0; i < n; i++)
...
template <>
gr_complex iir_filter<gr_complex, gr_complex, float>::filter(const gr_complex input);

template <>
gr_complex iir_filter<gr_complex, gr_complex, double>::filter(const gr_complex input);

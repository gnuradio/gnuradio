...
# Since there is no history, we don't need to pre-pad the signal
# with 0's 
def fir_filter(x, taps, decim=1):
    y = []
    for i in range(0, len(x)-len(taps), decim):
        yi = 0
        for j in range(len(taps)):
...
class test_freq_xlating_filter(gr_unittest.TestCase):
    ...
    def generate_ccf_source(self):
        ...
        self.fc = fc = 0.3
        self.bw = bw = 0.1
        self.taps = firdes.low_pass(1, fs, bw, bw / 4)
    ...
    def generate_ccc_source(self):
        ...
        self.fc = fc = 0.3
        self.bw = bw = 0.1
        self.taps = firdes.complex_band_pass(
            1, fs, -bw / 2, bw / 2, bw / 4)
    ...
    def test_fir_filter_ccf_001(self):
        ...
        # expected_data = expected_data[:-(len(expected_data) - len(self.taps) + 1)]
        ...
        self.assertComplexTuplesAlmostEqual(expected_data[(len(self.taps) - 1):], result_data, 5)

...
from gnuradio import gr, gr_unittest, blocks, streamops, filter
...
class test_moving_average(gr_unittest.TestCase):
    ...
    def test_vector_int(self):
        ...
        one_to_many = streamops.stream_to_streams(vlen)
        one_to_vector = streamops.stream_to_vector(vlen)
        many_to_vector = streamops.streams_to_vector(vlen)
        ...
        for idx, single in enumerate(isolated):
            tb.connect(one_to_many, idx, single, 0)
            tb.connect(single, 0, many_to_vector, idx)
    ...
    def test_vector_complex(self):
        ...
        one_to_vector = streamops.stream_to_vector(vlen)
        many_to_vector = streamops.streams_to_vector(vlen)
        isolated = [filter.moving_average_cc(100, 1) for i in range(vlen)]
        dut = filter.moving_average_cc(100, 1, vlen=vlen)
    ...
    def test_complex_scalar(self):
        ...
        history = 5  # num of samples to average

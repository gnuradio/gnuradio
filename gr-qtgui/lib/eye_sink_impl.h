...
#ifndef INCLUDED_QTGUI_EYE_SINK_IMPL_H
#define INCLUDED_QTGUI_EYE_SINK_IMPL_H
...
template <class T>
class QTGUI_API eye_sink_impl : public eye_sink<T>
{
private:
    void initialize();
    ...
    int d_start;
    int d_end;
    std::vector<volk::vector<T>> d_xbuffers;
...
public:
    eye_sink_impl(int size,
                  double samp_rate,
                  unsigned int nconnections,
                  QWidget* parent = NULL);
    ~eye_sink_impl() override;

    // Disallow copy/move because of the raw pointers.
    eye_sink_impl(const eye_sink_impl&) = delete;
    eye_sink_impl& operator=(const eye_sink_impl&) = delete;
    eye_sink_impl(eye_sink_impl&&) = delete;
    eye_sink_impl& operator=(eye_sink_impl&&) = delete;
...
#endif /* INCLUDED_QTGUI_EYE_SINK_IMPL_H */

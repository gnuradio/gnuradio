...
void matrix_display::initialize_mouse_actions()
{
    ...
    d_color_map_menu = new QMenu;
    d_color_map_menu->setTitle("Color Map");
    QAction* rgb = new QAction("RGB", this);
    connect(rgb, &QAction::triggered, this, [this]() { color_map("rgb"); });
    d_color_map_menu->addAction(rgb);
    QAction* indexed = new QAction("Indexed", this);
    connect(indexed, &QAction::triggered, this, [this]() { color_map("indexed"); });
    d_color_map_menu->addAction(indexed);
    QAction* alpha = new QAction("Alpha", this);
    connect(alpha, &QAction::triggered, this, [this]() { color_map("alpha"); });
    d_color_map_menu->addAction(alpha);
    d_menu->addMenu(d_color_map_menu);
}
...
{
    if (color_map == "rgb") {
        d_colorMap.setColorInterval(Qt::darkCyan, Qt::red);
        ...
        d_colorMap.setColorInterval(Qt::blue, Qt::red);
        d_colorMap.setFormat(QwtColorMap::Indexed);
        ...
        d_colorMap.setColorInterval(Qt::white, Qt::blue);
...
void matrix_display::color_map(const std::string& color_map)
{
    if (color_map == "rgb") {
        d_colorMap.setColorInterval(Qt::darkCyan, Qt::red);
        d_colorMap.addColorStop(0.1, Qt::cyan);

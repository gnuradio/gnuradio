...
template <>
void sink_impl<float>::initialize()
{
    if (qApp != NULL) {
...
template <>
void sink_impl<gr_complex>::initialize()
{
    if (qApp != NULL) {
        this->d_qApplication = qApp;
    } else {
...
template <class T>
typename sink<T>::sptr sink<T>::make(int fftsize,
                                     int wintype,
                                     double fc,
                                     double bw,
                                     ...
                                     QWidget* parent)
{
    return gnuradio::make_block_sptr<sink_impl<T>>(fftsize,
                                                   wintype,
                                                   fc,
                                                   bw,
                                                   name,
...
template <class T>
sink_impl<T>::sink_impl(int fftsize,
                        int wintype,
                        double fc,
                        double bw,
                        const std::string& name,
                        ...
                        bool plotconst,
                        QWidget* parent)
    : block("sink", io_signature::make(1, 1, sizeof(T)), io_signature::make(0, 0, 0)),
      d_fftsize(fftsize),
...
template <class T>
sink_impl<T>::~sink_impl()
{
}

template <class T>
bool sink_impl<T>::check_topology(int ninputs, int noutputs)
{
    return ninputs == 1;
}

template <class T>
void sink_impl<T>::forecast(int noutput_items, gr_vector_int& ninput_items_required)
{
...
template <class T>
void sink_impl<T>::exec_()
{
    this->d_qApplication->exec();
}

template <class T>
QWidget* sink_impl<T>::qwidget()
{
    return d_main_gui.qwidget();
}

template <class T>
void sink_impl<T>::set_fft_size(const int fftsize)
{
...
template <class T>
int sink_impl<T>::fft_size() const
{
    return d_fftsize;
}

template <class T>
void sink_impl<T>::set_frequency_range(const double centerfreq, const double bandwidth)
{
...
template <class T>
void sink_impl<T>::set_fft_power_db(double min, double max)
{
    d_main_gui.setFrequencyAxis(min, max);
}

template <class T>
void sink_impl<T>::enable_rf_freq(bool en)
{
...
template <class T>
void sink_impl<T>::set_update_time(double t)
{
    d_update_time = t;
...
template <class T>
void sink_impl<T>::fft(float* data_out, const T* data_in, int size)
{
    if (!d_window.empty()) {
...
template <class T>
void sink_impl<T>::check_clicked()
{
...
template <class T>
void sink_impl<T>::handle_set_freq(pmt::pmt_t msg)
{
    if (pmt::is_pair(msg)) {
...
template <>
int sink_impl<float>::general_work(int noutput_items,
...
template <>
int sink_impl<gr_complex>::general_work(int noutput_items,
                                        gr_vector_int& ninput_items,
                                        gr_vector_const_void_star& input_items,
                                        gr_vector_void_star& output_items)
{
...
template class sink<gr_complex>;

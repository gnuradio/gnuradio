...
#ifndef INCLUDED_QTGUI_FREQ_SINK_IMPL_H
#define INCLUDED_QTGUI_FREQ_SINK_IMPL_H
...
template <class T>
class QTGUI_API freq_sink_impl : public freq_sink<T>
{
private:
    void initialize();
...
public:
    freq_sink_impl(int size,
                   ...
                   int nconnections,
                   QWidget* parent = NULL);
    ~freq_sink_impl() override;

    // Disallow copy/move because of the raw pointers.
    freq_sink_impl(const freq_sink_impl&) = delete;
    freq_sink_impl& operator=(const freq_sink_impl&) = delete;
    freq_sink_impl(freq_sink_impl&&) = delete;
    freq_sink_impl& operator=(freq_sink_impl&&) = delete;
...
#endif /* INCLUDED_QTGUI_FREQ_SINK_IMPL_H */

...
eye_sink_c::sptr
eye_sink_c::make(int size, double samp_rate, unsigned int nconnections, QWidget* parent)
{
...
{
    if (nconnections > 12)
        throw std::runtime_error("eye_sink_c only supports up to 12 inputs");
...
void eye_sink_c_impl::initialize()
{
    if (qApp != NULL) {
        d_qApplication = qApp;
    } else {
...
void eye_sink_c_impl::set_y_label(const std::string& label, const std::string& unit)
{
    d_main_gui->setYLabel(label, unit);
}

void eye_sink_c_impl::set_update_time(double t)
{
...
void eye_sink_c_impl::set_line_color(unsigned int which, const std::string& color)
{
    d_main_gui->setLineColor(which, color.c_str());
}

void eye_sink_c_impl::set_line_width(unsigned int which, int width)
{
    d_main_gui->setLineWidth(which, width);
}

void eye_sink_c_impl::set_line_style(unsigned int which, int style)
{
    d_main_gui->setLineStyle(which, (Qt::PenStyle)style);
}

void eye_sink_c_impl::set_line_marker(unsigned int which, int marker)
{
    d_main_gui->setLineMarker(which, (QwtSymbol::Style)marker);
}

void eye_sink_c_impl::set_line_alpha(unsigned int which, double alpha)
{
    d_main_gui->setMarkerAlpha(which, (int)(255.0 * alpha));
}

void eye_sink_c_impl::set_trigger_mode(gr::qtgui::trigger_mode mode,
...
void eye_sink_c_impl::set_size(int width, int height)
{
    d_main_gui->resize(QSize(width, height));
}

std::string eye_sink_c_impl::title() { return d_main_gui->title().toStdString(); }

std::string eye_sink_c_impl::line_label(unsigned int which)
{
    return d_main_gui->lineLabel(which).toStdString();
}

std::string eye_sink_c_impl::line_color(unsigned int which)
{
    return d_main_gui->lineColor(which).toStdString();
}

int eye_sink_c_impl::line_width(unsigned int which)
{
    return d_main_gui->lineWidth(which);
}

int eye_sink_c_impl::line_style(unsigned int which)
{
    return d_main_gui->lineStyle(which);
}

int eye_sink_c_impl::line_marker(unsigned int which)
{
    return d_main_gui->lineMarker(which);
}

double eye_sink_c_impl::line_alpha(unsigned int which)
{
    return (double)(d_main_gui->markerAlpha(which)) / 255.0;
}

void eye_sink_c_impl::set_nsamps(const int newsize)
{
    if (newsize != d_size) {
        gr::thread::scoped_lock lock(d_setlock);
...
void eye_sink_c_impl::set_samp_rate(const double samp_rate)
{
    gr::thread::scoped_lock lock(d_setlock);
    d_samp_rate = samp_rate;
    d_main_gui->setSampleRate(d_samp_rate);
...
int eye_sink_c_impl::nsamps() const { return d_size; }

void eye_sink_c_impl::enable_menu(bool en) { d_main_gui->enableMenu(en); }

void eye_sink_c_impl::enable_grid(bool en) { d_main_gui->setGrid(en); }

void eye_sink_c_impl::enable_autoscale(bool en) { d_main_gui->autoScale(en); }

void eye_sink_c_impl::enable_stem_plot(bool en)
{
    /* Used by parent class, do not remove*/
}

void eye_sink_c_impl::enable_semilogx(bool en)
{
    /* Used by parent class, do not remove*/
}

void eye_sink_c_impl::enable_semilogy(bool en)
{
    /* Used by parent class, do not remove*/
}
...
void eye_sink_c_impl::enable_tags(unsigned int which, bool en)
{
    d_main_gui->setTagMenu(which, en);
}

void eye_sink_c_impl::enable_tags(bool en)
{
...
void eye_sink_c_impl::enable_axis_labels(bool en) { d_main_gui->setAxisLabels(en); }

void eye_sink_c_impl::disable_legend() { d_main_gui->disableLegend(); }

void eye_sink_c_impl::reset()
{
    gr::thread::scoped_lock lock(d_setlock);
    _reset();
}

void eye_sink_c_impl::_reset()
{
    unsigned int n;
    if (d_trigger_delay) {
...
void eye_sink_c_impl::_npoints_resize()
{
    int newsize = d_main_gui->getNPoints();
    set_nsamps(newsize);
}

void eye_sink_c_impl::_adjust_tags(int adj)
{
...
void eye_sink_c_impl::_gui_update_trigger()
{
...
void eye_sink_c_impl::_test_trigger_tags(int nitems)
{
    int trigger_index;
...
void eye_sink_c_impl::_test_trigger_norm(int nitems, gr_vector_const_void_star inputs)
{
    int trigger_index;
...
bool eye_sink_c_impl::_test_trigger_slope(const gr_complex* in) const
{
    float x0, x1;
...
int eye_sink_c_impl::work(int noutput_items,
...
void eye_sink_c_impl::handle_pdus(pmt::pmt_t msg)
{
    size_t len;
    ...
    if (pmt::is_c32vector(samples)) {
        ...
        throw std::runtime_error("eye_sink_c: unknown data type "

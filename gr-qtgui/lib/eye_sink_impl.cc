...
template <class T>
typename eye_sink<T>::sptr
eye_sink<T>::make(int size, double samp_rate, unsigned int nconnections, QWidget* parent)
{
    return gnuradio::make_block_sptr<eye_sink_impl<T>>(
        size, samp_rate, nconnections, parent);
}

template <>
void eye_sink_impl<float>::initialize()
{
    if (qApp != NULL) {
...
template <>
void eye_sink_impl<gr_complex>::initialize()
{
    if (qApp != NULL) {
        this->d_qApplication = qApp;
    } else {
...
template <>
void eye_sink_impl<float>::_reset()
{
    unsigned int n;
    if (d_trigger_delay) {
        for (n = 0; n < d_nconnections; n++) {
            ...
            memmove(d_xbuffers[n].data(),
                    &d_xbuffers[n][d_end - d_trigger_delay],
...
template <>
void eye_sink_impl<gr_complex>::_reset()
{
    unsigned int n;
    if (d_trigger_delay) {
...
template <>
void eye_sink_impl<float>::set_nsamps(const int newsize)
{
    if (newsize != d_size) {
        ...
        for (unsigned int n = 0; n < d_nconnections + 1; n++) {
            ...
            d_xbuffers[n].clear();
            d_xbuffers[n].resize(d_buffer_size);
        }

        // If delay was set beyond the new boundary, pull it back.
        if (d_trigger_delay > 2 * d_sps) {
            this->d_logger->warn("Trigger delay ({:g}) outside of display range "
...
template <>
void eye_sink_impl<gr_complex>::set_nsamps(const int newsize)
{
    if (newsize != d_size) {
        gr::thread::scoped_lock lock(this->d_setlock);
...
template <>
void eye_sink_impl<float>::_test_trigger_tags(int nitems)
{
    int trigger_index;
    ...
    this->get_tags_in_range(
        tags, d_trigger_channel, nr, nr + nitems + 1, d_trigger_tag_key);
    if (!tags.empty()) {
...
template <>
void eye_sink_impl<gr_complex>::_test_trigger_tags(int nitems)
{
    int trigger_index;

    uint64_t nr = this->nitems_read(d_trigger_channel / 2);
    std::vector<gr::tag_t> tags;
    this->get_tags_in_range(
...
template <>
bool eye_sink_impl<float>::_test_trigger_slope(const float* in) const
{
    float x0, x1;
    x0 = in[0];
    x1 = in[1];
...
template <>
bool eye_sink_impl<gr_complex>::_test_trigger_slope(const gr_complex* in) const
{
    float x0, x1;
...
template <>
void eye_sink_impl<float>::_test_trigger_norm(int nitems,
...
template <>
void eye_sink_impl<gr_complex>::_test_trigger_norm(int nitems,
                                                   gr_vector_const_void_star inputs)
{
    int trigger_index;
...
template <>
void eye_sink_impl<float>::handle_pdus(pmt::pmt_t msg)
{
    size_t len;
    ...
    if (pmt::is_f32vector(samples)) {
        ...
        throw std::runtime_error("eye_sink<T>: unknown data type "
...
template <>
void eye_sink_impl<gr_complex>::handle_pdus(pmt::pmt_t msg)
{
    size_t len;
    pmt::pmt_t dict, samples;
...
template <>
eye_sink_impl<float>::eye_sink_impl(int size,
                                    double samp_rate,
...
{
    if (nconnections > 24)
        throw std::runtime_error("eye_sink only supports up to 24 inputs");
    ...
    for (unsigned int n = 0; n < d_nconnections + 1; n++) {
        ...
        d_xbuffers.emplace_back(d_buffer_size);
    }
    ...
    this->set_trigger_mode(gr::qtgui::TRIG_MODE_FREE, gr::qtgui::TRIG_SLOPE_POS, 0, 0, 0);

    this->set_history(2);          // so we can look ahead for the trigger slope
    this->declare_sample_delay(1); // delay the tags for a history of 2
}

template <>
eye_sink_impl<gr_complex>::eye_sink_impl(int size,
                                         double samp_rate,
                                         unsigned int nconnections,
                                         QWidget* parent)
    : sync_block("eye_sink",
...
{
    if (nconnections > 12)
        throw std::runtime_error("eye_sink only supports up to 12 inputs");
...
template <>
bool eye_sink_impl<float>::check_topology(int ninputs, int noutputs)
{
    return (unsigned int)(ninputs) == d_nconnections;
}

template <>
bool eye_sink_impl<gr_complex>::check_topology(int ninputs, int noutputs)
{
    return (unsigned int)(2 * ninputs) == d_nconnections;
}

template <class T>
void eye_sink_impl<T>::exec_()
{
    this->d_qApplication->exec();
}

template <class T>
QWidget* eye_sink_impl<T>::qwidget()
{
    return d_main_gui;
}

template <class T>
void eye_sink_impl<T>::set_y_axis(double min, double max)
{
    d_main_gui->setYaxis(min, max);
}

template <class T>
void eye_sink_impl<T>::set_y_label(const std::string& label, const std::string& unit)
{
    d_main_gui->setYLabel(label, unit);
}

template <class T>
void eye_sink_impl<T>::set_update_time(double t)
{
...
template <class T>
void eye_sink_impl<T>::set_samp_per_symbol(unsigned int sps)
{
    d_main_gui->setSamplesPerSymbol(sps);
}

template <class T>
void eye_sink_impl<T>::set_line_label(unsigned int which, const std::string& label)
{
    d_main_gui->setLineLabel(which, label.c_str());
}

template <class T>
void eye_sink_impl<T>::set_line_color(unsigned int which, const std::string& color)
{
    d_main_gui->setLineColor(which, color.c_str());
}

template <class T>
void eye_sink_impl<T>::set_line_width(unsigned int which, int width)
{
    d_main_gui->setLineWidth(which, width);
}

template <class T>
void eye_sink_impl<T>::set_line_style(unsigned int which, int style)
{
    d_main_gui->setLineStyle(which, (Qt::PenStyle)style);
}

template <class T>
void eye_sink_impl<T>::set_line_marker(unsigned int which, int marker)
{
    d_main_gui->setLineMarker(which, (QwtSymbol::Style)marker);
}

template <class T>
void eye_sink_impl<T>::set_line_alpha(unsigned int which, double alpha)
{
    d_main_gui->setMarkerAlpha(which, (int)(255.0 * alpha));
}

template <class T>
void eye_sink_impl<T>::set_trigger_mode(gr::qtgui::trigger_mode mode,
                                        ...
                                        const std::string& tag_key)
{
    ...
    if ((d_trigger_delay < 0) || (d_trigger_delay > 2 * d_sps)) {
        this->d_logger->warn("Trigger delay ({:g}) outside of display range (0:{:g}).",
                             d_trigger_delay / d_samp_rate,
                             (2 * d_sps) / d_samp_rate);
...
template <class T>
void eye_sink_impl<T>::set_size(int width, int height)
{
    d_main_gui->resize(QSize(width, height));
}

template <class T>
std::string eye_sink_impl<T>::title()
{
    return d_main_gui->title().toStdString();
}

template <class T>
std::string eye_sink_impl<T>::line_label(unsigned int which)
{
    return d_main_gui->lineLabel(0).toStdString();
}

template <class T>
std::string eye_sink_impl<T>::line_color(unsigned int which)
{
    return d_main_gui->lineColor(0).toStdString();
}

template <class T>
int eye_sink_impl<T>::line_width(unsigned int which)
{
    return d_main_gui->lineWidth(0);
}

template <class T>
int eye_sink_impl<T>::line_style(unsigned int which)
{
    return d_main_gui->lineStyle(0);
}

template <class T>
int eye_sink_impl<T>::line_marker(unsigned int which)
{
    return d_main_gui->lineMarker(0);
}

template <class T>
double eye_sink_impl<T>::line_alpha(unsigned int which)
{
    return (double)(d_main_gui->markerAlpha(0)) / 255.0;
}

template <class T>
void eye_sink_impl<T>::set_samp_rate(const double samp_rate)
{
    gr::thread::scoped_lock lock(this->d_setlock);
    d_samp_rate = samp_rate;
    d_main_gui->setSampleRate(d_samp_rate);
...
template <class T>
int eye_sink_impl<T>::nsamps() const
{
    return d_size;
}

template <class T>
void eye_sink_impl<T>::enable_stem_plot(bool en)
{
    /* Used by parent class, do not remove*/
}

template <class T>
void eye_sink_impl<T>::enable_menu(bool en)
{
    d_main_gui->enableMenu(en);
}

template <class T>
void eye_sink_impl<T>::enable_grid(bool en)
{
    d_main_gui->setGrid(en);
}

template <class T>
void eye_sink_impl<T>::enable_autoscale(bool en)
{
    d_main_gui->autoScale(en);
}

template <class T>
void eye_sink_impl<T>::enable_semilogx(bool en)
{
    /* Used by parent class, do not remove*/
}

template <class T>
void eye_sink_impl<T>::enable_semilogy(bool en)
{
    /* Used by parent class, do not remove*/
}

template <class T>
void eye_sink_impl<T>::enable_control_panel(bool en)
{
    if (en)
...
template <class T>
void eye_sink_impl<T>::enable_tags(unsigned int which, bool en)
{
    d_main_gui->setTagMenu(which, en);
}

template <class T>
void eye_sink_impl<T>::enable_tags(bool en)
{
...
template <class T>
void eye_sink_impl<T>::enable_axis_labels(bool en)
{
    d_main_gui->setAxisLabels(en);
}

template <class T>
void eye_sink_impl<T>::disable_legend()
{
    d_main_gui->disableLegend();
}

template <class T>
void eye_sink_impl<T>::reset()
{
    gr::thread::scoped_lock lock(this->d_setlock);
    _reset();
}
...
template <class T>
void eye_sink_impl<T>::_npoints_resize()
{
...
template <class T>
void eye_sink_impl<T>::_adjust_tags(int adj)
{
...
template <class T>
void eye_sink_impl<T>::_gui_update_trigger()
{
    ...
    if (delay != d_trigger_delay) {
        ...
        if ((delay < 0) || (delay > 2 * d_sps)) {
            this->d_logger->warn(
                "Trigger delay ({:g}) outside of display range (0:{:g}).",
                delay / d_samp_rate,
                (2 * d_sps) / d_samp_rate);
...
template <>
int eye_sink_impl<float>::work(int noutput_items,
                               ...
                               gr_vector_void_star& output_items)
{
    ...
    for (n = 0; n < d_nconnections; n++) {
        ...
        memcpy(&d_xbuffers[n][d_index], &in[1], nitems * sizeof(float));

        uint64_t nr = this->nitems_read(idx);
        std::vector<gr::tag_t> tags;
        this->get_tags_in_range(tags, idx, nr, nr + nitems);
    ...
    if ((d_triggered) && (d_index == d_end)) {
        ...
        for (n = 0; n < d_nconnections; n++) {
            volk_32f_convert_64f(d_buffers[n].data(), &d_xbuffers[n][d_start], d_size);
        }
        ...
        if (gr::high_res_timer_now() - d_last_time > d_update_time) {
            ...
            this->d_qApplication->postEvent(
                d_main_gui, new TimeUpdateEvent(d_buffers, d_size, d_tags));
        }
...
template <>
int eye_sink_impl<gr_complex>::work(int noutput_items,
                                    gr_vector_const_void_star& input_items,
                                    gr_vector_void_star& output_items)
{
    unsigned int n = 0;
    ...
    for (n = 0; n < d_nconnections / 2; n++) {
        ...
        memcpy(&d_xbuffers[n][d_index], &in[1], nitems * sizeof(gr_complex));

        uint64_t nr = this->nitems_read(n);
        std::vector<gr::tag_t> tags;
        this->get_tags_in_range(tags, n, nr, nr + nitems);
        for (size_t t = 0; t < tags.size(); t++) {
            tags[t].offset = tags[t].offset - nr + (d_index - d_start - 1);
        }
...
template class eye_sink<float>;
template class eye_sink<gr_complex>;

...
template <class T>
typename time_sink<T>::sptr time_sink<T>::make(int size,
                                               double samp_rate,
                                               const std::string& name,
                                               unsigned int nconnections,
                                               QWidget* parent)
{
    return gnuradio::make_block_sptr<time_sink_impl<T>>(
        size, samp_rate, name, nconnections, parent);
}

template <>
void time_sink_impl<float>::initialize()
{
    if (qApp != NULL) {
...
template <>
void time_sink_impl<gr_complex>::initialize()
{
    if (qApp != NULL) {
        this->d_qApplication = qApp;
    } else {
...
template <>
void time_sink_impl<float>::_reset()
{
    unsigned int n;
    if (d_trigger_delay) {
...
template <>
void time_sink_impl<gr_complex>::_reset()
{
    unsigned int n;
    if (d_trigger_delay) {
...
template <>
bool time_sink_impl<float>::_test_trigger_slope(const float* in) const
{
    float x0, x1;
    x0 = in[0];
    x1 = in[1];
...
template <>
bool time_sink_impl<gr_complex>::_test_trigger_slope(const gr_complex* in) const
{
    float x0, x1;
...
template <>
void time_sink_impl<float>::set_nsamps(const int newsize)
{
    if (newsize != d_size) {
...
template <>
void time_sink_impl<gr_complex>::set_nsamps(const int newsize)
{
    if (newsize != d_size) {
        gr::thread::scoped_lock lock(this->d_setlock);
...
template <>
void time_sink_impl<float>::handle_pdus(pmt::pmt_t msg)
{
    size_t len;
    ...
    if (pmt::is_f32vector(samples)) {
        ...
        throw std::runtime_error("time_sink: unknown data type "
                                 "of samples; must be float or complex.");
    }
...
template <>
void time_sink_impl<gr_complex>::handle_pdus(pmt::pmt_t msg)
{
    size_t len;
    pmt::pmt_t dict, samples;
    std::vector<std::vector<gr::tag_t>> t(1);
...
template <>
time_sink_impl<float>::time_sink_impl(int size,
                                      double samp_rate,
                                      const std::string& name,
                                      unsigned int nconnections,
                                      QWidget* parent)
...
{
    if (nconnections > 24)
        throw std::runtime_error("time_sink only supports up to 24 inputs");
    ...
    this->set_trigger_mode(TRIG_MODE_FREE, TRIG_SLOPE_POS, 0, 0, 0);
...
template <>
time_sink_impl<gr_complex>::time_sink_impl(int size,
                                           double samp_rate,
                                           const std::string& name,
                                           unsigned int nconnections,
                                           QWidget* parent)
...
template <class T>
time_sink_impl<T>::~time_sink_impl()
{
...
template <class T>
bool time_sink_impl<T>::check_topology(int ninputs, int noutputs)
{
    return (unsigned int)(ninputs) == d_nconnections;
}

template <class T>
void time_sink_impl<T>::exec_()
{
    this->d_qApplication->exec();
}

template <class T>
QWidget* time_sink_impl<T>::qwidget()
{
    return d_main_gui;
}

template <class T>
void time_sink_impl<T>::set_y_axis(double min, double max)
{
    d_main_gui->setYaxis(min, max);
}

template <class T>
void time_sink_impl<T>::set_y_label(const std::string& label, const std::string& unit)
{
    d_main_gui->setYLabel(label, unit);
}

template <class T>
void time_sink_impl<T>::set_update_time(double t)
{
...
template <class T>
void time_sink_impl<T>::set_title(const std::string& title)
{
    d_main_gui->setTitle(title.c_str());
}

template <class T>
void time_sink_impl<T>::set_line_label(unsigned int which, const std::string& label)
{
    d_main_gui->setLineLabel(which, label.c_str());
}

template <class T>
void time_sink_impl<T>::set_line_color(unsigned int which, const std::string& color)
{
    d_main_gui->setLineColor(which, color.c_str());
}

template <class T>
void time_sink_impl<T>::set_line_width(unsigned int which, int width)
{
    d_main_gui->setLineWidth(which, width);
}

template <class T>
void time_sink_impl<T>::set_line_style(unsigned int which, int style)
{
    d_main_gui->setLineStyle(which, (Qt::PenStyle)style);
}

template <class T>
void time_sink_impl<T>::set_line_marker(unsigned int which, int marker)
{
    d_main_gui->setLineMarker(which, (QwtSymbol::Style)marker);
}

template <class T>
void time_sink_impl<T>::set_line_alpha(unsigned int which, double alpha)
{
    d_main_gui->setMarkerAlpha(which, (int)(255.0 * alpha));
}

template <class T>
void time_sink_impl<T>::set_trigger_mode(trigger_mode mode,
                                         trigger_slope slope,
                                         float level,
                                         float delay,
                                         int channel,
...
template <class T>
void time_sink_impl<T>::set_size(int width, int height)
{
    d_main_gui->resize(QSize(width, height));
}

template <class T>
std::string time_sink_impl<T>::title()
{
    return d_main_gui->title().toStdString();
}

template <class T>
std::string time_sink_impl<T>::line_label(unsigned int which)
{
    return d_main_gui->lineLabel(which).toStdString();
}

template <class T>
std::string time_sink_impl<T>::line_color(unsigned int which)
{
    return d_main_gui->lineColor(which).toStdString();
}

template <class T>
int time_sink_impl<T>::line_width(unsigned int which)
{
    return d_main_gui->lineWidth(which);
}

template <class T>
int time_sink_impl<T>::line_style(unsigned int which)
{
    return d_main_gui->lineStyle(which);
}

template <class T>
int time_sink_impl<T>::line_marker(unsigned int which)
{
    return d_main_gui->lineMarker(which);
}

template <class T>
double time_sink_impl<T>::line_alpha(unsigned int which)
{
    return (double)(d_main_gui->markerAlpha(which)) / 255.0;
}

template <class T>
void time_sink_impl<T>::set_samp_rate(const double samp_rate)
{
    gr::thread::scoped_lock lock(this->d_setlock);
    d_samp_rate = samp_rate;
    d_main_gui->setSampleRate(d_samp_rate);
...
template <class T>
int time_sink_impl<T>::nsamps() const
{
    return d_size;
}

template <class T>
void time_sink_impl<T>::enable_stem_plot(bool en)
{
    d_main_gui->setStem(en);
}

template <class T>
void time_sink_impl<T>::enable_menu(bool en)
{
    d_main_gui->enableMenu(en);
}

template <class T>
void time_sink_impl<T>::enable_grid(bool en)
{
    d_main_gui->setGrid(en);
}

template <class T>
void time_sink_impl<T>::enable_autoscale(bool en)
{
    d_main_gui->autoScale(en);
}

template <class T>
void time_sink_impl<T>::enable_semilogx(bool en)
{
    d_main_gui->setSemilogx(en);
}

template <class T>
void time_sink_impl<T>::enable_semilogy(bool en)
{
    d_main_gui->setSemilogy(en);
}

template <class T>
void time_sink_impl<T>::enable_control_panel(bool en)
{
    if (en)
...
template <class T>
void time_sink_impl<T>::enable_tags(unsigned int which, bool en)
{
    d_main_gui->setTagMenu(which, en);
}

template <class T>
void time_sink_impl<T>::enable_tags(bool en)
{
...
template <class T>
void time_sink_impl<T>::enable_axis_labels(bool en)
{
    d_main_gui->setAxisLabels(en);
}

template <class T>
void time_sink_impl<T>::disable_legend()
{
    d_main_gui->disableLegend();
}

template <class T>
void time_sink_impl<T>::reset()
{
    gr::thread::scoped_lock lock(this->d_setlock);
    _reset();
}
...
template <class T>
void time_sink_impl<T>::_npoints_resize()
{
...
template <class T>
void time_sink_impl<T>::_adjust_tags(int adj)
{
...
template <class T>
void time_sink_impl<T>::_gui_update_trigger()
{
...
template <>
void time_sink_impl<float>::_test_trigger_tags(int nitems)
{
    int trigger_index;
...
template <>
void time_sink_impl<gr_complex>::_test_trigger_tags(int nitems)
{
    int trigger_index;
...
template <>
void time_sink_impl<float>::_test_trigger_norm(int nitems,
...
template <>
void time_sink_impl<gr_complex>::_test_trigger_norm(int nitems,
                                                    gr_vector_const_void_star inputs)
{
    int trigger_index;
...
template <>
int time_sink_impl<float>::work(int noutput_items,
                                ...
                                gr_vector_void_star& output_items)
{
    ...
    for (n = 0; n < d_nconnections; n++) {
        ...
        std::vector<gr::tag_t> tags;
        this->get_tags_in_range(tags, idx, nr, nr + nitems + 1);
...
template <>
int time_sink_impl<gr_complex>::work(int noutput_items,
                                     gr_vector_const_void_star& input_items,
                                     gr_vector_void_star& output_items)
{
    unsigned int n = 0;
    ...
    for (n = 0; n < d_nconnections / 2; n++) {
        ...
        std::vector<gr::tag_t> tags;
        this->get_tags_in_range(tags, n, nr, nr + nitems + 1);
        for (size_t t = 0; t < tags.size(); t++) {
            tags[t].offset = tags[t].offset - nr + (d_index - d_start - 1);
        }

...
void freq_sink_c_impl::buildwindow()
{
    ...
    GR_LOG_INFO(d_logger, boost::format("norm factor: %1%") % norm_factor);
    GR_LOG_INFO(d_logger, boost::format("win before: %1%") % d_window.at(10));
    // Now normalize such that the power of d_window is 1
    std::for_each(d_window.begin(), d_window.end(), [&](float& x) { x /= norm_factor; });
    GR_LOG_INFO(d_logger, boost::format("win after: %1%") % d_window.at(10));
}

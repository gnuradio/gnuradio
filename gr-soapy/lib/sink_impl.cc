...
sink_impl::sink_impl(size_t nchan,
                     ...
                     const std::string& length_tag_name)
    : gr::sync_block(
          "sink", args_to_io_sig(type, nchan), gr::io_signature::make(0, 0, 0)),
      d_stopped(true),
      ...
      d_burst_remaining(0)
{
    ...
    for (size_t i = 0; i < d_nchan; i++) {
        ...
        if ((d_sampling_rate < minRate) || (d_sampling_rate > maxRate)) {
            std::string msg = name() +
                              ": Unsupported sample rate. Rate must be between " +
    ...
    SoapySDR::ArgInfoList supported_args = d_device->getStreamArgsInfo(SOAPY_SDR_TX, 0);
 ...
 */
sink_impl::~sink_impl() { stop(); }

void sink_impl::register_msg_cmd_handler(const pmt::pmt_t& cmd, cmd_handler_t handler)
{
    d_cmd_handlers[cmd] = handler;
}

bool sink_impl::DC_offset_support(int channel)
{
    return d_device->hasDCOffset(SOAPY_SDR_TX, channel);
}

bool sink_impl::IQ_balance_support(int channel)
{
    return d_device->hasIQBalance(SOAPY_SDR_TX, channel);
}

bool sink_impl::freq_correction_support(int channel)
{
    return d_device->hasFrequencyCorrection(SOAPY_SDR_TX, channel);
}

void sink_impl::set_frequency(size_t channel, double frequency)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_frequency(size_t channel, const std::string& name, double frequency)
{
    if (channel >= d_nchan) {
        return;
    }
...
bool sink_impl::is_gain_valid(size_t channel, std::string gainType)
{
...
void sink_impl::set_gain(size_t channel, const std::string name, float gain)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_gain(size_t channel, float gain)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_gain_mode(size_t channel, bool gain_auto_mode)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_sample_rate(size_t channel, double sample_rate)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_bandwidth(size_t channel, double bandwidth)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_antenna(const size_t channel, const std::string& name)
{
    if (channel >= d_nchan) {
        return;
    }
    ...
    if (antennaList.size() > 0) {
        if (std::find(antennaList.begin(), antennaList.end(), name) ==
            antennaList.end()) {
            GR_LOG_WARN(d_logger, boost::format("Antenna name %s not supported.") % name);
            return;
        }
    }
...
void sink_impl::set_dc_offset(size_t channel,
                              ...
                              bool dc_offset_auto_mode)
{
    ...
    if (!dc_offset_auto_mode && d_device->hasDCOffset(SOAPY_SDR_TX, channel)) {
        d_device->setDCOffset(SOAPY_SDR_TX, channel, dc_offset);
        d_dc_offset = dc_offset;
    }
}

void sink_impl::set_dc_offset_mode(size_t channel, bool dc_offset_auto_mode)
{
    if (channel >= d_nchan) {
        return;
    }
    ...
    if (dc_offset_auto_mode && d_device->hasDCOffsetMode(SOAPY_SDR_TX, channel)) {
        d_device->setDCOffsetMode(SOAPY_SDR_TX, channel, dc_offset_auto_mode);
        d_dc_offset_auto_mode = dc_offset_auto_mode;
    }
}

void sink_impl::set_frequency_correction(size_t channel, double freq_correction)
{
    if (channel >= d_nchan) {
        return;
    }
    ...
    if (d_device->hasFrequencyCorrection(SOAPY_SDR_TX, channel)) {
        d_device->setFrequencyCorrection(SOAPY_SDR_TX, channel, freq_correction);
        d_frequency_correction = freq_correction;
    }
}

void sink_impl::set_iq_balance(size_t channel, gr_complexd iq_balance)
{
    if (channel >= d_nchan) {
        return;
    }
...
void sink_impl::set_master_clock_rate(double clock_rate)
{
    d_device->setMasterClockRate(clock_rate);
    d_clock_rate = clock_rate;
}

void sink_impl::set_clock_source(const std::string& clock_source)
{
    d_device->setClockSource(clock_source);
    d_clock_source = clock_source;
}

void sink_impl::set_frontend_mapping(const std::string& mapping)
{
    d_device->setFrontendMapping(SOAPY_SDR_TX, mapping);
}

double sink_impl::get_frequency(size_t channel)
{
    return d_device->getFrequency(SOAPY_SDR_TX, channel);
}

double sink_impl::get_gain(size_t channel)
{
    return d_device->getGain(SOAPY_SDR_TX, channel);
}

bool sink_impl::get_gain_mode(size_t channel)
{
    return d_device->getGainMode(SOAPY_SDR_TX, channel);
}

double sink_impl::get_sampling_rate(size_t channel)
{
    return d_device->getSampleRate(SOAPY_SDR_TX, channel);
}

double sink_impl::get_bandwidth(size_t channel)
{
    return d_device->getBandwidth(SOAPY_SDR_TX, channel);
}

std::string sink_impl::get_antenna(size_t channel)
{
    return d_device->getAntenna(SOAPY_SDR_TX, channel);
}

std::complex<double> sink_impl::get_dc_offset(size_t channel)
{
    return d_device->getDCOffset(SOAPY_SDR_TX, channel);
}

bool sink_impl::get_dc_offset_mode(size_t channel)
{
    return d_device->getDCOffsetMode(SOAPY_SDR_TX, channel);
}

double sink_impl::get_frequency_correction(size_t channel)
{
    return d_device->getFrequencyCorrection(SOAPY_SDR_TX, channel);
}

std::complex<double> sink_impl::get_iq_balance(size_t channel)
{
    return d_device->getIQBalance(SOAPY_SDR_TX, channel);
}

double sink_impl::get_master_clock_rate() { return d_device->getMasterClockRate(); }

std::string sink_impl::get_clock_source() { return d_device->getClockSource(); }

void sink_impl::cmd_handler_frequency(pmt::pmt_t val, size_t chann)
{
    set_frequency(chann, pmt::to_float(val));
}

void sink_impl::cmd_handler_gain(pmt::pmt_t val, size_t chann)
{
    set_gain(chann, pmt::to_float(val));
}

void sink_impl::cmd_handler_samp_rate(pmt::pmt_t val, size_t chann)
{
    set_sample_rate(chann, pmt::to_float(val));
}

void sink_impl::cmd_handler_bw(pmt::pmt_t val, size_t chann)
{
    set_bandwidth(chann, pmt::to_float(val));
}

void sink_impl::cmd_handler_antenna(pmt::pmt_t val, size_t chann)
{
...
void sink_impl::tag_work(int noutput_items)
{
...
void sink_impl::msg_handler_command(pmt::pmt_t msg)
{
    if (!pmt::is_dict(msg)) {
        return;
    }

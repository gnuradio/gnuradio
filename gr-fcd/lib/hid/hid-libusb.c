...
static int get_usage(uint8_t* report_descriptor,
                     size_t size,
                        ...
                        if ((vendor_id == 0x0 || vendor_id == dev_vid) &&
                            ...
                            if (res >= 0) {
...
#ifdef INVASIVE_GET_USAGE
                                {
                                    ...
                                    res = libusb_kernel_driver_active(handle,
                                                                      interface_num);
                                    if (res == 1) {
                                        res = libusb_detach_kernel_driver(handle,
                                                                          interface_num);
                                        if (res < 0)
                                            LOG("Couldn't detach kernel driver, even "
                                                "though a kernel driver was attached.");
                                        else
                                            detached = 1;
                                    }
#endif
                                    ...
                                    if (res >= 0) {
                                        ...
                                        if (res >= 0) {
                                            ...
                                            LOG("libusb_control_transfer() for getting "
                                                "the HID report failed with %d\n",
                                                res);
                                    ...
                                    if (detached) {
                                        res = libusb_attach_kernel_driver(handle,
                                                                          interface_num);
                                        if (res < 0)
                ...
                if (intf_desc->bInterfaceClass == LIBUSB_CLASS_HID) {
                    ...
                    if (!strcmp(dev_path, path)) {
                        ...
                        if (libusb_kernel_driver_active(
                                dev->device_handle, intf_desc->bInterfaceNumber) == 1) {
                            res = libusb_detach_kernel_driver(
                                dev->device_handle, intf_desc->bInterfaceNumber);
                            if (res < 0) {
...
HID_API_EXPORT const wchar_t* HID_API_CALL hid_error(hid_device* dev) { return NULL; }

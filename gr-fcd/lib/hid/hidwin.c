...
static hid_device* new_hid_device()
{
    ...
    dev->ol.hEvent =
        CreateEvent(NULL, FALSE, FALSE /*initial state f=nonsignaled*/, NULL);
...
hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
    ...
    for (;;) {
        ...
        res = SetupDiGetDeviceInterfaceDetailA(
            device_info_set, &device_interface_data, NULL, 0, &required_size, NULL);
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_manufacturer_string(hid_device* dev,
                                                                 wchar_t* string,
                                                                 size_t maxlen)
{
    BOOL res;
    ...
    res = HidD_GetManufacturerString(
        dev->device_handle, string, sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS));
    if (!res) {
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_product_string(hid_device* dev,
                                                            wchar_t* string,
                                                            size_t maxlen)
{
    BOOL res;
...
int HID_API_EXPORT_CALL HID_API_CALL hid_get_serial_number_string(hid_device* dev,
                                                                  wchar_t* string,
                                                                  size_t maxlen)
{
    BOOL res;

...
#define FCD_CMD_APP_SET_FREQ_HZ                                                    \
    101 /*!< Send with 4 byte unsigned little endian frequency in Hz, returns with \
           actual frequency set in Hz */
#define FCD_CMD_APP_GET_FREQ_HZ \
    102 /*!< Returns 4 byte unsigned little endian frequency in Hz. */

#define FCD_CMD_APP_GET_IF_RSSI                                                         \
    104 /*!< Supposed to return 1 byte unsigned IF RSSI (-35dBm=0, -10dBm=70) but it is \
           not functional. */
...
#define FCD_CMD_APP_GET_DC_CORR                                                       \
    107 /*!< Returns 2 byte unsigned I DC correction followed by 2 byte unsigned Q DC \
           correction. 32768 is the default centre value. */
#define FCD_CMD_APP_SET_IQ_CORR                                                         \
    108 /*!< Send with 2 byte signed phase correction followed by 2 byte unsigned gain  \
           correction. 0 is the default centre value for phase correction, 32768 is the \
           default centre value for gain. */
#define FCD_CMD_APP_GET_IQ_CORR                                                         \
    109 /*!< Returns 2 byte signed phase correction followed by 2 byte unsigned gain    \
           correction. 0 is the default centre value for phase correction, 32768 is the \
           default centre value for gain. */

#define FCD_CMD_APP_SET_LNA_GAIN                           \
    110 /*!< Send a 1 byte value, see enums for reference. \
         */
...
#define FCD_CMD_APP_SET_BIAS_TEE \
    126 /*!< Bias T for ext LNA. Send with one byte: 1=ON, 0=OFF. */
...
typedef enum { TBE_VHF2, TBE_VHF3, TBE_UHF, TBE_LBAND } TUNER_BAND_ENUM;
...
typedef enum { TIG1E_N3_0DB = 0, TIG1E_P6_0DB = 1 } TUNER_IF_GAIN1_ENUM;

typedef enum { TIGME_LINEARITY = 0, TIGME_SENSITIVITY = 1 } TUNER_IF_GAIN_MODE_ENUM;
...
typedef enum { TIG4E_P0_0DB = 0, TIG4E_P1_0DB = 1, TIG4E_P2_0DB = 2 } TUNER_IF_GAIN4_ENUM;

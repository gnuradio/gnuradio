...
#            self.update_group_curves()
    ...
    def detach_firstattached(self, plot):
        ...
        if len(items) > 2:
            ...
            if plot is self.mplots['mFREQ']:
                items[1].setPen(QtGui.QPen(
                    QtCore.Qt.red, 1, QtCore.Qt.SolidLine))
                self.set_actgrid()
    ...
    def action_save_dialog(self):
        file_dialog_output = QtGui.QFileDialog.getSaveFileName(
    ...
    def action_open_dialog(self):
        file_dialog_output = QtGui.QFileDialog.getOpenFileName(

...
void amp_experiment_destroy(struct AEXP* aexp)
{
    ...
    if (aexp->var != 0.0)
        printf("var...: %4.3f  std dev...: %4.3f (%d amplitude samples)\n",
               aexp->var / aexp->var_n,
               sqrt(aexp->var / aexp->var_n),
               aexp->var_n);
    if (aexp->vq_var != 0.0)
        printf("vq var: %4.3f  std dev...: %4.3f (%d amplitude samples)\n",
               aexp->vq_var / aexp->vq_var_n,
               sqrt(aexp->vq_var / aexp->vq_var_n),
               aexp->vq_var_n);
    free(aexp);
}
...
static void
add_quant_noise(struct AEXP* aexp, MODEL* model, int start, int end, float sigma_dB)
{
    int m;
    float x_dB;
    float noise_sam_dB;
...
static void print_sparse_pred_error(struct AEXP* aexp, MODEL* model, float mag_thresh)
{
    ...
    if (mag > mag_thresh) {
        ...
        for (m = 1; m <= model->L; m++) {
            ...
            error =
                PRED_COEFF * 20.0 * log10(aexp->A_prev[m]) - 20.0 * log10(model->A[m]);
...
static int split_vq(float sparse_pe_out[],
                    struct AEXP* aexp,
                    struct codebook* vq,
                    float weights[],
                    float sparse_pe_in[])
{
    ...
    vq_ind = vq_amp(
        vq->cb, &sparse_pe_in[vq->offset], &weights[vq->offset], vq->k, vq->m, &se);
...
static void sparse_vq_pred_error(struct AEXP* aexp, MODEL* model)
{
    int m, index;
...
static void
split_error(struct AEXP* aexp, struct codebook* vq, float sparse_pe_in[], int ind)
{
    int i, j;
...
/* gain/shape vq search.  Returns index of best gain.  Gain is additive (as we use log
 * quantisers) */

int gain_shape_vq_amp(
    float cb[], float vec[], float weights[], int d, int e, float* se, float* best_gain)
{
    float error;      /* current error		*/
    int besti;        /* best index so far		*/
    float best_error; /* best error so far		*/
...
static void gain_shape_split_vq(float sparse_pe_out[],
                                struct AEXP* aexp,
                                struct codebook* vq,
                                float weights[],
                                float sparse_pe_in[],
                                float* best_gain)
{
    ...
    vq_ind = gain_shape_vq_amp(vq->cb,
                               &sparse_pe_in[vq->offset],
                               &weights[vq->offset],
                               vq->k,
                               vq->m,
                               &se,
                               best_gain);
    // printf("\n offset %d k %d m %d vq_ind %d gain: %4.2f j: ", vq->offset, vq->k,
    // vq->m, vq_ind, *best_gain);
...
static void gain_shape_sparse_vq_amp(struct AEXP* aexp, MODEL* model)
{
    ...
    gain_shape_split_vq(
        sparse_pe_out, aexp, aexp->vq1, weights, sparse_pe_in, &best_gain);
    gain_shape_split_vq(
        sparse_pe_out, aexp, aexp->vq2, weights, sparse_pe_in, &best_gain);
    gain_shape_split_vq(
        sparse_pe_out, aexp, aexp->vq3, weights, sparse_pe_in, &best_gain);
    gain_shape_split_vq(
        sparse_pe_out, aexp, aexp->vq4, weights, sparse_pe_in, &best_gain);
...
static void interp_split_vq(float sparse_pe_out[],
                            struct AEXP* aexp,
                            struct codebook* vq,
                            float sparse_pe_in[],
                            int ind)
...
static float bins[] = {
    ...
    2000.0, 2400.0, 2800.0, 3000.0, 3400.0, 3600.0, 4000.0
};

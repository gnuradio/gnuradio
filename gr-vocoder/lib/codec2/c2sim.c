...
void synth_one_frame(kiss_fft_cfg fft_inv_cfg,
                     short buf[],
                     MODEL* model,
                     float Sn_[],
                     float Pn[],
...
int main(int argc, char* argv[])
{
        ...
        case 0:
            if (strcmp(long_options[option_index].name, "lpc") == 0) {
                ...
                if ((fvoicing = fopen(optarg, "rt")) == NULL) {
                    fprintf(stderr,
                            "Error opening voicing file: %s: %s.\n",
                            optarg,
                            strerror(errno));
                    exit(1);
                }
                ...
                if ((fjvm = fopen(optarg, "wt")) == NULL) {
                    fprintf(stderr,
                            "Error opening pitch & energy dump file: %s: %s.\n",
                            optarg,
                            strerror(errno));
                    exit(1);
                }
    ...
    */
    ...
    fft_fwd_cfg =
        kiss_fft_alloc(FFT_ENC, 0, NULL, NULL); /* fwd FFT,used in several places   */
    fft_inv_cfg =
        kiss_fft_alloc(FFT_DEC, 1, NULL, NULL); /* inverse FFT, used just for synth */
...
//	   model.Wo, prev_uq_Wo);
        ...
        if (lpc_model) {
            ...
            */
            ...
            if (!lsp && !lspd && !lspvq && !lspres && !lspjvm && !lspanssi && !lspjnd &&
                !lspmel)
                for (i = 0; i < LPC_ORD; i++)
                    lsps_[i] = lsps[i];
            ...
            if (scalar_quant_Wo_e) {
                ...
                if (decimate) {
                    ...
                    if (dt && ((frames % 4) == 0)) {
                        ...
                        model.Wo =
                            decode_Wo_dt(encode_Wo_dt(model.Wo, prev__Wo), prev__Wo);
                    } else
                        model.Wo = decode_Wo(encode_Wo(model.Wo));
                }
                ...
                */
                ...
                if (phase0)
                    phase_synth_zero_order(
                        fft_fwd_cfg, &interp_model, ak_interp, ex_phase, order);
                if (postfilt)
                    postfilter(&interp_model, &bg_est);
                synth_one_frame(
                    fft_inv_cfg, buf, &interp_model, Sn_, Pn, prede, &de_mem, gain);
                // printf("  buf[0] %d\n", buf[0]);
                if (fout != NULL)

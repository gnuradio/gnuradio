...
void phase_experiment_destroy(struct PEXP* pexp)
{
    ...
    if (pexp->var != 0.0)
        printf("var...: %4.3f  std dev...: %4.3f (%d non zero phases)\n",
               pexp->var / pexp->var_n,
               sqrt(pexp->var / pexp->var_n),
               pexp->var_n);
    if (pexp->vq_var != 0.0)
        printf("vq var: %4.3f  vq std dev: %4.3f (%d non zero phases)\n",
               pexp->vq_var / pexp->vq_var_n,
               sqrt(pexp->vq_var / pexp->vq_var_n),
               pexp->vq_var_n);
    free(pexp);
}
...
static void
print_pred_error(struct PEXP* pexp, MODEL* model, int start, int end, float mag_thresh)
{
    int i;
    float mag;
...
static float refine_Wo(struct PEXP* pexp, MODEL* model, int start, int end);
...
static void
top_amp(struct PEXP* pexp, MODEL* model, int start, int end, int n_harm, int pred)
{
...
#define ALTTOP
        ...
        if (top) {
            ...
            model->phi[i] = PI * (1.0 - 2.0 * (float)rand() /
                                            RAND_MAX); // note: try rand for higher harms
            removed++;
        }
#else
        if (!top) {
...
static void
limit_prediction_error(struct PEXP* pexp, MODEL* model, int start, int end, float limit)
{
    int i;
...
static void
quant_prediction_error(struct PEXP* pexp, MODEL* model, int start, int end, float limit)
{
    int i;
...
static void print_sparse_pred_error(
    struct PEXP* pexp, MODEL* model, int start, int end, float mag_thresh)
{
    int i, index;
...
static void split_vq(COMP sparse_pe_out[],
                     struct PEXP* pexp,
                     struct codebook* vq,
                     float weights[],
                     COMP sparse_pe_in[])
...
static void sparse_vq_pred_error(struct PEXP* pexp, MODEL* model)
{
    int i, index;

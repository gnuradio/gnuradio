...
#define NFILTERTIMING \
    (M + NFILTER + M) /* filter memory used for resampling after timing estimation */
...
#define NPILOTBASEBAND \
    (NPILOTCOEFF + M + M / P) /* number of pilot baseband samples reqd for pilot LPF */
...
void tx_filter(COMP tx_baseband[NC + 1][M],
               ...
               COMP tx_filter_memory[NC + 1][NSYM]);
void fdm_upconvert(
    COMP tx_fdm[], int Nc, COMP tx_baseband[NC + 1][M], COMP phase_tx[], COMP freq_tx[]);
...
void lpf_peak_pick(float* foff,
                   ...
                   COMP S[],
                   int nin);
void freq_shift(COMP rx_fdm_fcorr[],
                COMP rx_fdm[],
                float foff,
                COMP* foff_rect,
                COMP* foff_phase_rect,

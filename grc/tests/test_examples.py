...
BLOCK_PATHS = [path.abspath(path.join(ROOT, "grc/blocks")), path.abspath(path.join(ROOT, "build/gr-uhd/grc"))]
        ...
        if path.isdir(file_dir) and file_dir.name.startswith("gr-") and not file_dir.name.startswith("gr-trellis"):
            try:
                for pos_ex in os.scandir(path.join(file_dir, "examples")):
                    if (
                        path.isfile(pos_ex)
                        and pos_ex.name.endswith(".grc")
                        and not path.basename(pos_ex) in BLACKLISTED
                    ):
                        ...
                        for pos_ex2 in os.scandir(pos_ex):
                            if (
                                path.isfile(pos_ex2)
                                and pos_ex2.name.endswith(".grc")
                                and not path.basename(pos_ex2) in BLACKLISTED
                            ):

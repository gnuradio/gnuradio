    ...
    def __call__(self, parameter=None):
        if self.type and parameter:
            ...
            try:
                ...
                raise TypeError(
                    "Invalid parameter type for action '{}'. Expected: '{}'".format(self.get_name(), self.type)
                )
        else:
            self.activate()
...
TOGGLE_SHOW_CODE_PREVIEW_TAB = actions.register(
    ...
    tooltip="Show a preview of the code generated for each Block in its " "Properties Dialog",
...
RELOAD_BLOCKS = actions.register(
    "app.reload_blocks", label="Reload _Blocks", tooltip="Reload Blocks", icon_name="view-refresh"
)

...
class Action(Gio.SimpleAction):
    ...
    def __init__(
        ...
        if keypresses:
            self.keypresses = [
                kp.replace("<Ctrl>", get_modifier_key(True)) for kp in keypresses
            ]
        else:
    ...
    def __call__(self, parameter=None):
        if self.type and parameter:
            ...
            try:
                ...
                raise TypeError(
                    "Invalid parameter type for action '{}'. Expected: '{}'".format(
                        self.get_name(), self.type
                    )
                )
        else:
            self.activate()
...
TOGGLE_SHOW_GRID = actions.register(
    "win.show_grid",
    label="Show Grid",

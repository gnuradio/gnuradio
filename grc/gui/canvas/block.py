...
class Block(CoreBlock, Drawable):
    ...
    def create_labels(self, cr=None):
        ...
        if not self.is_dummy_block:
            markups = [
                ...
                if (param.hide not in ("all", "part") or (param.dtype == "id" and force_show_id))
            ]
        else:
            markups = [
                '<span font_desc="{font}"><b>key: </b>{key}</span>'.format(font=Constants.PARAM_FONT, key=self.key)
            ]
        ...
        def get_min_height_for_ports(ports):
            ...
            if any([p.dtype == "bus" for p in ports]):
                min_height = (
                    ...
                    + sum(port.height + Constants.PORT_SPACING for port in ports if port.dtype == "bus")
        ...
        height = max(height, get_min_height_for_ports(self.active_sinks), get_min_height_for_ports(self.active_sources))
    ...
    def get_extents(self):
        ...
        for port in self.active_ports():
            extent = (
                min_or_max(xy, offset + p_xy)
                for offset, min_or_max, xy, p_xy in zip((x, y, x, y), (min, min, max, max), extent, port.get_extents())
            )
        return tuple(extent)

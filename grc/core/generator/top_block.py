...
class TopBlockGenerator(object):
    ...
    def _blocks(self):
        ...
        blocks = [
            ...
            for b in fg.blocks
            if b.enabled
            and not (b.get_bypassed() or b.is_import or b.is_snippet or b in parameters or b.key == "options")
        ]
    ...
    def _connections(self):
        fg = self._flow_graph
        templates = {key: Template(text) for key, text in fg.parent_platform.connection_templates.items()}
        ...
        virtual_connections = [
            ...
            if (isinstance(c.source_block, blocks.VirtualSource) or isinstance(c.sink_block, blocks.VirtualSink))
        ]

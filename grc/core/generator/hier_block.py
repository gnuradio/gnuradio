...
class HierBlockGenerator(TopBlockGenerator):
    ...
    def _build_block_n_from_flow_graph_io(self):
        ...
        data["id"] = block_id
        data["label"] = (
            self._flow_graph.get_option("title") or self._flow_graph.get_option("id").replace("_", " ").title()
        )
        ...
        t["imports"] = "from {0} import {0}  # grc-generated hier_block".format(self._flow_graph.get_option("id"))
        # Make data
        if parameters:
            t["make"] = "{cls}(\n    {kwargs},\n)".format(
                cls=block_id,

...
class Validator(object):
    ...
    def _check_var_key_dict(self, data, key_type, value_scheme):
        for key, value in data.items():
            if not isinstance(key, key_type):
                self._error("Key type {!r} for {!r} not in valid types".format(type(value).__name__, key))
            if isinstance(value_scheme, Spec):
                self._check_dict(value, value_scheme)
            elif not isinstance(value, value_scheme):
                self._error("Value type {!r} for {!r} not in valid types".format(type(value).__name__, key))
    ...
    def _check_value(self, value, types_, item_scheme, label):
        if not isinstance(value, types_):
            self._error("Value type {!r} for {!r} not in valid types".format(type(value).__name__, label))
        if item_scheme:

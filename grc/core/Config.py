...
class Config(object):
    ...
    def block_paths(self):
        ...
        collected_paths = sum((paths.split(os.pathsep) for paths in paths_sources), [])

        valid_paths = [normpath(expanduser(expandvars(path))) for path in collected_paths if exists(path)]

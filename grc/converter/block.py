...
def convert_block_xml(node):
    converter = cheetah_converter.Converter(
        names={
    ...
    data["parameters"] = [
        convert_param_xml(param_node, converter.to_python_dec) for param_node in node.iterfind("param")
    ] or no_value
    # data['params'] = {p.pop('key'): p for p in data['params']}

    data["inputs"] = [
        convert_port_xml(port_node, converter.to_python_dec) for port_node in node.iterfind("sink")
    ] or no_value

    data["outputs"] = [
        convert_port_xml(port_node, converter.to_python_dec) for port_node in node.iterfind("source")
    ] or no_value
    data["value"] = converter.to_python_dec(node.findtext("var_value")) or (
        "${ value }" if block_id.startswith("variable") else no_value
    )

    data["asserts"] = [converter.to_python_dec(check_node.text) for check_node in node.iterfind("check")] or no_value
...
def convert_templates(node, convert, block_id=""):
    ...
    imports = "\n".join(convert(import_node.text) for import_node in node.iterfind("import"))
    if "\n" in imports:
    ...
    templates["callbacks"] = [convert(cb_node.text) for cb_node in node.iterfind("callback")] or no_value

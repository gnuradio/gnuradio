                                            ...
                                            unsigned long buffer_size)
{
    ...
    return gnuradio::get_initial_sptr(new fmcomms2_source_impl(
...
void fmcomms2_source_impl::set_samplerate(unsigned long samplerate)
{
    ...
    std::string filt_config(d_filter_source);
    if (filt_config.compare("Off") == 0) {
        ...
        int ret = ad9361_set_bb_rate_custom_filter_manual(
            phy, samplerate, d_fpass, d_fstop, d_bandwidth, d_bandwidth);
        if (ret) {
            throw std::runtime_error("Unable to set BB rate");
        }
    } else
...
//         boost::to_string(samplerate)); params.push_back("in_voltage_rf_bandwidth=" +
